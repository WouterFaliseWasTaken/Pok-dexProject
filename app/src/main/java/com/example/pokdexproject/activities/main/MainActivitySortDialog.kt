...
class MainActivitySortDialog(context: Context) : Dialog(context){
    ...
    private fun setUpListeners(viewModel: MainViewModel) {
        ...
        findViewById<Button>(R.id.sort_dialog_apply_button).setOnClickListener{
          /**
            when (findViewById<ChipGroup>(R.id.sort_option_group).checkedChipId){
                R.id.sort_by_id_ascending_check -> viewModel.setSortBy((Pair(Criterion.ID,true)))
                R.id.sort_by_id_descending_check -> viewModel.setSortBy((Pair(Criterion.ID,false)))
                R.id.sort_by_name_ascending_check -> viewModel.setSortBy((Pair(Criterion.NAME,true)))
                R.id.sort_by_name_desending_check -> viewModel.setSortBy((Pair(Criterion.NAME,false)))
            }
            */
            dismiss()
        }
